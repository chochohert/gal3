...
class Register_dialog extends JDialog {
	private JTable tab_lecture;
	private JScrollPane scr_tab_lecture;
	private JPanel pane_south;
	private DefaultTableModel model;
	private String[] title = { "과정 번호", "과정명", "강사", "교재", "강의실", "수강료", "평일/주말", "반", "주간/야간", "신청 만료일" };
	private JButton btn_submit, btn_close;
	private String student_no;

	public Register_dialog(JTable tab_student) {
	...
	private void display() {
		...
		String sql = "select l.lecture_no, l.lecture_name, t.teacher_name, b.book_name, l.room_no, l.lec_fee, l.week, l.class, l.time, l.eroll_end_date from lecture l "
				...
				+ "where (l.week, l.time) not in(select l.week, l.time from regist r join lecture l on r.lecture_no = l.lecture_no where student_no = ?)";
		try {
			...
			while (rs.next()) {
				...
				String week = null;
				if (rs.getString(7).equals("Y")||rs.getString(7).equals("y")) {
					week = "평일";
				} else {
					week = "주말";
				}
				data[6] = week; // 주중/주말
//				data[6] = rs.getString(7); // 주중/주말
